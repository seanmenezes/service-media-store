package com.plutotv.common.layer.step.v1.search.series;

import com.plutotv.common.layer.endpoint.v1.search.SearchEndpointsV1;
import com.plutotv.common.model.response.v1.search.series.Series;
...
public class SearchSeriesSteps {
    ...
    @Step
    public String getV1SearchSeriesMostRecentlyUpdatedSeriesTimestamp() {
        Response response = searchEndpointsV1.getSearchSeriesLastModifiedDateTime();
        response.then().statusCode(SC_NO_CONTENT);
        return response.getHeader(LAST_MODIFIED_HEADER);
    }

    @Step
    public Response getV1SearchSeriesFullSync(String syncStop) {
        Map<String, String> requestParams = new HashMap<>();
        requestParams.put(REPLICATION_TYPE, "full-sync");
        requestParams.put(UPDATED_BEFORE, syncStop);
        return searchEndpointsV1.getV1SearchSeries(requestParams);
    }

    @Step
    public SearchSeriesDatum getFirstSeries(Response v1SearchSeries, String replicationType) {
        return v1SearchSeries.as(Series.class).getData().stream()
                .filter(series -> Objects.equals(replicationType, series.getMeta().getReplicaAction()))
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException(
                        format("There is no series with replication type %s for /v1/search/series", replicationType)));
    }
}
