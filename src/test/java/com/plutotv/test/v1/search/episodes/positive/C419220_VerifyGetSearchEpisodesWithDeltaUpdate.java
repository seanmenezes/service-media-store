...
public class C419220_VerifyGetSearchEpisodesWithDeltaUpdate extends BaseTest {
    private final RequestHelper requestHelper = new RequestHelper();

    @Test(groups = {"full"})
    public void verifyGetSearchEpisodesWithDeltaUpdate() {
        HashMap<String, String> params = new HashMap<>();
        params.put(REPLICATION_TYPE, "delta-update");
        params.put(UPDATED_AFTER, String.valueOf(Instant.now().minus(30, ChronoUnit.DAYS)));
        ...
        List<String> replicaActions = response.then().extract().path("data.meta.replicaAction");

        if (replicaActions.contains("upsert")) {
            response.then()
                    .body("data.meta.replicaAction", not(hasItems(not(anyOf(is("upsert"), is("remove"))))))
